...
import com.tiger.item.pojo.*;
import com.tiger.service.mapper.*;
...
//    private AmqpTemplate amqpTemplate;
    ...
    public PageResult<Spu> querySpuByPage(Integer page, Integer rows, String key, Boolean saleable) {
        ...
        if (CollectionUtils.isEmpty(spuList)) {
            throw new TigerException(ExceptionEnum.SPU_NOT_FOUND);
        }
        //对查询结果中的分类名和品牌名进行处理
    ...
    public List<Sku> querySkuBySpuId(Long spuId) {
        ...
        if (CollectionUtils.isEmpty(skuList)) {
            throw new TigerException(ExceptionEnum.SKU_NOT_FOUND);
        }
...
//        sendMessage(spuId, "delete");
    ...
    public void addGoods(Spu spu) {
        ...
        if (count != 1) {
            throw new TigerException(ExceptionEnum.GOODS_SAVE_ERROR);
        }
        //插入spuDetail数据
...
//        sendMessage(spu.getId(), "insert");
    ...
    public void updateGoods(Spu spu) {
        ...
        int count = spuMapper.updateByPrimaryKeySelective(spu);
        if (count == 0) {
            throw new TigerException(ExceptionEnum.GOODS_UPDATE_ERROR);
        }
...
//        sendMessage(spu.getId(), "update");
    ...
    public void handleSaleable(Spu spu) {
        ...
        int count = spuMapper.updateByPrimaryKeySelective(spu);
        if (count != 1) {
            throw new TigerException(ExceptionEnum.UPDATE_SALEABLE_ERROR);
        }
    }
    ...
    public List<Sku> querySkusByIds(List<Long> ids) {
        ...
        if (CollectionUtils.isEmpty(skus)) {
            throw new TigerException(ExceptionEnum.GOODS_NOT_FOUND);
        }
        //填充库存
        fillStock(ids, skus);
        return skus;
...
//    }
    ...
    private void fillStock(List<Long> ids, List<Sku> skus) {
        ...
        if (CollectionUtils.isEmpty(stocks)) {
            throw new TigerException(ExceptionEnum.STOCK_NOT_FOUND);
        }
